...
// Also may be for testing css separately later.
...
firebase.auth().onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        // unique user id provided by auth is stored in user.uid - this will be tracked in
        // redux store to track authentication for page access.
        // user.uid is created when user clicks login btn on Loginpage which fires startLogin action in actions/auth.js, and
        // firebase.auth signs them in with google, then the id is stored in the store with the login action fired by startLogin
